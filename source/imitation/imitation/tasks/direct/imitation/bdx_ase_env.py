...
from .bdx_ase_env_cfg import BdxAseEnvCfg
from imitation.tasks.utils import MotionLoader


class BdxAseEnv(DirectRLEnv):
    cfg: BdxAseEnvCfg

    def __init__(self, cfg: BdxAseEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.latents = torch.zeros((self.num_envs, self.cfg.latent_dim), dtype=torch.float32, device=self.device)
        self.latent_reset_steps = torch.zeros((self.num_envs,), dtype=torch.long, device=self.device)
    ...
    def _get_observations(self) -> dict:
        ...
        policy_obs = torch.cat((amp_obs, self.latents), dim=-1)
    ...
    def _reset_latents(self, env_ids: torch.Tensor):
        self.latents[env_ids] = self._sample_latents(len(env_ids))
        latent_step_offsets = torch.randint(
            low=self.cfg.latent_steps_min,
            high=self.cfg.latent_steps_max + 1,
            size=(len(env_ids),),
            device=self.device,
        )
        self.latent_reset_steps[env_ids] = self.episode_length_buf[env_ids] + latent_step_offsets
    ...
    def _sample_latents(self, n: int) -> torch.Tensor:
        z = torch.randn((n, self.cfg.latent_dim), dtype=torch.float32, device=self.device)
        return F.normalize(z, dim=-1)

    def _update_latents(self):
        env_ids = torch.nonzero(self.episode_length_buf >= self.latent_reset_steps, as_tuple=False).flatten()
        if len(env_ids) > 0:
            self._reset_latents(env_ids)
